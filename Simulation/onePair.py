...
spyRtn = np.log(spy_data[1:60]) - np.log(spy_data[0:59])
mmmRtn = np.log(mmm_data[1:60]) - np.log(mmm_data[0:59])

beta0, beta1, epsilon = regression(spyRtn, mmmRtn)
#print epsilon
sscore = score(epsilon, 40)
print sscore
plt.plot(range(0,len(sscore)),sscore,'ro')
plt.axis([0,len(sscore),-2,2])
plt.show()